...
interface ImageCarouselOrbitProps {
  images: ImageCard[]
  className?: string
}

export function ImageCarouselOrbit({ images, className }: ImageCarouselOrbitProps) {
  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      setRotatingCards((prev) => prev.map((value) => (value + (isHovering ? 0.2 : 0.5)) % 360))
    }, 50)
  ...
  const containerClass = cn("relative w-full h-96 sm:h-[500px]", className)
  ...
  return (
    <div className="relative w-full min-h-screen bg-gradient-to-b from-background via-background to-background overflow-hidden">
      ...
      <div className="relative z-10 flex flex-col items-center justify-center min-h-screen px-4 sm:px-6 lg:px-8">
        {/* Carousel Container */}
        <ImageCarouselOrbit images={images} className="w-full max-w-6xl mb-12 sm:mb-16" />

        {/* Content Section */}
        <div className="relative z-20 text-center max-w-2xl mx-auto mb-12 sm:mb-16">
          <h1 className="text-4xl sm:text-5xl lg:text-6xl font-serif font-bold text-foreground mb-4 sm:mb-3 text-balance leading-tight">
            {title}
          </h1>

          {subtitle && (
            <p className="text-base sm:text-lg text-primary uppercase tracking-[0.3em] mb-4">
              {subtitle}
            </p>
          )}
